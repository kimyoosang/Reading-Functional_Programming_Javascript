...
- 예 1-2: 이름 리스트를 읽고 데이터를 정제 후, 중복은 제거하고 정렬하는 코드 (함수형)
...
- 흐름 중심으로 코딩하면 재사용성, 모듈화 측면에서도 당연히 유리하다

## **3.6 마치며**

- 고계 함수 map, reduce, filter를 쓰면 코드를 확장할 수 있다
- 로대시JS는 데이터 흐름과 변환 과정이 명확히 구획된 제어 체인을 통해 데이터 처리 및 프로그램 작성을 도모한다
- 함수형 프로그래밍의 선언적 스타일로 개발하면 코드를 헤아리기 쉽다
